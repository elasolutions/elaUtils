package org.elasolutions.utils.test.performance;

import org.elasolutions.utils.test.EventStatus;
import org.elasolutions.utils.test.PrintTestExecutionListener;

public class PerformancePauseCancelDemo {

    public static void main(final String[] args) {
        System.out.println("Start: PerformanceSingleActionDemo");
        System.out.println("---------------------------------------------");

        final long start = System.currentTimeMillis();

        final PerformancePauseCancelDemo test = new PerformancePauseCancelDemo();
        test.run();

        System.out.println("---------------------------------------------");
        System.out.println("Runtime: " + (System.currentTimeMillis() - start));
        System.out.println("Finished.");
        System.exit(0);
    }

    private void run() {
        Thread pauseThread = new Thread() {
            @Override
            public void run() {
                try {
                    while(loadExecutor==null) {
                        Thread.sleep(100);
                    }

                    System.out.println("-------------------\r\nPause\r\n--------------------");
                    loadExecutor.pauseTest();

                    System.out.println("-------------------\r\nSleep\r\n--------------------");
                    Thread.sleep(4000);

                    System.out.println("-------------------\r\nResume\r\n--------------------");
                    loadExecutor.resumeTest();

                    System.out.println("-------------------\r\nSleep\r\n--------------------");
                    Thread.sleep(4000);

                    System.out.println("-------------------\r\nCancel\r\n--------------------");
                    loadExecutor.cancelTest();
                } catch (Exception excep) {
                    // TODO: handle exception
                }

            }
        };
        pauseThread.start();


        ///////////////////////////////////////////////////////////////////////////////////////////
        // Determine how you want the class to be executed.
        final String testName = "service call test";

        final int numberOfThreads = 10;

        final int numberOfActionsPerThread = 100;

        final long maxRuntime = -1; // no limit on the runtime

        final long delayBetweenActionsInMilliseconds = 100;

        final boolean useRandomDelay = true;


        ///////////////////////////////////////////////////////////////////////////////////////////
        // 1 action, multiple threads against the action
        loadExecutor = TestCaseExecutor.newTest(testName, numberOfActionsPerThread, maxRuntime,
            delayBetweenActionsInMilliseconds, useRandomDelay);

        loadExecutor.run(new TestActionImpl(), numberOfThreads, new PrintTestExecutionListener());



//        try { Thread.sleep(2000); } catch (InterruptedException excep) {}
//
//        System.out.println("-------------------\r\nCancel\r\n--------------------");
//        load.cancelTest();

        System.out.println("-------------------\r\nSleep\r\n--------------------");
        try { Thread.sleep(4000); } catch (InterruptedException excep) {}

        // display results
//        for (TestThread result : load.getTestList()) {
//            System.out.println(result.toString());
//            for( EventDetailResult details : result.getResults() ) {
//                System.out.println("\t" + details.toString());
//            }
//        }

        TestResults results = loadExecutor.getResults();
        System.out.println("Total runtime = " + results.getTestDuration());
        System.out.println("Average thread execution time= " + results.getAverageThreadTime());

    }

    ///////////////////////////////////////////////////////////////////////////////////////////
    // Test: This is the test that will be executed.   The action() contains the code to be executed
    class  TestActionImpl implements TestAction {
        @Override
        public EventStatus action(int threadId, int count, final StringBuffer optionalResultMessage) {
            //-----------------------------------------------
            // The count corresponds to the number of times this method has been called.
            // count is useful for things like logging or pull records from a list or file.

            //-----------------------------------------------
            // do something here.
            optionalResultMessage.append("complete");

            // The thread is to simulate activity
            try {
                Thread.sleep(20);
            } catch (InterruptedException excep) {
                excep.printStackTrace();
            }

            //-----------------------------------------------
            // return the status or message of the test.
            return EventStatus.Success;
        }

        @Override
        public void cleanup() {
            // do some clean up here, such as closing database connections.
        }
    }

    TestCaseExecutor loadExecutor;
}
